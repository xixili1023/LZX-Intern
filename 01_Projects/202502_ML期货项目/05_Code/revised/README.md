...
- `02_Data/processed/features.pkl`：由已有上游数据处理流程生成。
- `02_Data/processed/trading_calendar.csv`：真实交易日历，至少包含 `trade_date`。
- Python 环境及 Notebook 中导入的依赖。

## 输出

- 数据集与质量报告：`02_Data/processed/revised/`
- 模型与回测中间结果：`05_Code/revised/artifacts/`
...
- 运行结果应同步记录到 [[../../06_Experiment/experiment_log|Experiment Log]]。
