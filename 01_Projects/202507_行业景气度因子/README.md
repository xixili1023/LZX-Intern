...
- 原文中的定性绩效描述保留为“原始材料陈述”，需要后续用回测文件和结果指标核验。
- 原始文档继续作为迁移依据；项目内各模块是后续维护入口。

## 待确认与未归类

- 龙头股滚动评估窗口原文写为“约 6 个月”，精确交易日数量待确认。
- TopK 持仓数量、统一费率和部分回测参数未在原文给出。
- 景气度指标的完整字段表、Wind EDB 序列代码、发布时间与具体滞后天数待补充。
- 原文提到多个测试和参数比较，但缺少实验日期、Experiment ID、完整参数与结果表。
- 原文中的 `mei` 疑似误输入，未作为研究内容迁移。
